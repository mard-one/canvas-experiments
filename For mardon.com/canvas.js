...
Goal.prototype.step = function() {
  this.ax += rand(-0.04, 0.04);
  this.ay += rand(-0.04, 0.04);
  this.vx += this.ax;
  this.vy += this.ay;
...
function reset() {
  ...
  goal.x = canvas.width;
  goal.y = canvas.height + 10;
  followers.forEach(function(follower, i) {
    follower.x = canvas.width;
    follower.y = canvas.height + 10;
  });
}
setInterval(function(){
  var random1 = rand(-0.5, 0.5);
  var random2 = rand(-0.5, 0.5);
  console.log("rand1", random1);
  console.log("rand2", random2);
  if(random1 >= 0){
    this.x = 0
  } else {
    console.log("else1 works");
    this.x = canvas.width
